...
const SnakeGame = () => {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setSnake(prev => {
        ...
        // Self collision
        if (prev.some((segment, index) => index > 0 && segment.x === newHead.x && segment.y === newHead.y)) {
          setGameOver(true);
          clearInterval(intervalRef.current);
          return prev;
        }
  ...
  const resetGame = () => {
    ...
    axios.get('http://localhost:8080/api/snake-questions/random?count=5')
      .then(res => {
        setQuestions(res.data);
        const correctAnswers = res.data.map(q => 
          q.answers.find(a => a.correct)?.answer
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/3">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-4">
              <h2 className="text-3xl font-bold text-center text-gray-800 mb-4">Snake Quiz</h2>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                <h4 className="font-medium text-yellow-800 mb-1">How to play:</h4>
          ...
          <div className="lg:w-2/3">
            <div className="bg-white rounded-2xl shadow-lg p-6">
              {(gameOver || gameWon) && (
                <div className={`absolute inset-0 flex items-center justify-center z-10 bg-black bg-opacity-50 rounded-2xl ${
