...
const SnakeQuestionForm = () => {
  ...
  useEffect(() => {
    if (bookId && !bookTitle) {
      setLoading(true);
      axios.get(`http://ec2-3-25-81-177.ap-southeast-2.compute.amazonaws.com:3000/api/books/${bookId}`)
        .then(response => {
  ...
  useEffect(() => {
    if (bookId) {
      ...
      axios.get(`http://ec2-3-25-81-177.ap-southeast-2.compute.amazonaws.com:3000/api/snake-questions/book/${bookId}`, {
        headers: {
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const promises = questionsToSubmit.map(q => 
      axios.post('http://ec2-3-25-81-177.ap-southeast-2.compute.amazonaws.com:3000/api/snake-questions', q, { headers })
    );

    await Promise.all(promises);
    
    // Instead of reloading, fetch the newly created questions and update state
    const response = await axios.get(`http://ec2-3-25-81-177.ap-southeast-2.compute.amazonaws.com:3000/api/snake-questions/book/${bookId}`, { headers });
    setExistingQuestions(response.data);
    setHasExistingQuestions(true);
  ...
  const handleUpdateQuestion = async (questionID) => {
    try {
      ...
      await axios.put(
        `http://ec2-3-25-81-177.ap-southeast-2.compute.amazonaws.com:3000/api/snake-questions/${questionID}`,
        {
